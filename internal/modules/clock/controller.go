...
func (ctrl *Controller) CreateEntry(c *gin.Context) {
	var req schema.CreateClockEntryRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		response.Err(c, 400, err.Error())
		return
	}
	...
	response.OK(c, 201, entry)
}

func (ctrl *Controller) DeleteEntry(c *gin.Context) {
	id, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
		response.Err(c, 400, "invalid id")
		return
	...
	response.OK(c, 200, gin.H{"deleted": true})
}
